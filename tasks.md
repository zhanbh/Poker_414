...
- 立棍只有一轮：从随机/继承的首牌候选开始逐人选择立棍或跳过，全部跳过才按原候选正常开打；有人立棍后先给其队友抢立，确定立棍方后仅对方逐人反立。立棍/反立改变首牌后再锁主。每队最多一人，其队友弃牌；无超时自动选择。
...
- 本局进行中禁止重新开始，至少等本局结算。
- 报爆：整手可一次出完时提示，公开标识但不泄露牌面。报爆后仍可正常拆牌首出/跟牌；拆牌后若剩余两张及以上再次满足爆牌条件，重新提示确认，剩最后一张可单出的牌则自动爆。对子可报爆，实际是否为差由回应场景判定。
- 记录无倒计时的活动时间；连续 1 分钟无操作只产生暂离标识，不自动过牌、不暂停、不判负。重连保留原座位和牌局状态。
...
**Test scenarios:**
- Happy path: 4 人入座后首局随机首牌候选只产生一次，四人依次立棍/跳过；全部跳过进入普通，立棍后队友可抢立，确定立棍方后仅对方反立，最终首牌后锁主；下一手由上一手第一个出完牌者首出。
...
- Happy path: 玩家剩余 `444`、`3456`、`334455` 或 `44A` 时可以报爆；报爆后仍可拆出合法子集跟牌或首出，拆牌后若再次满足爆牌条件则重新确认。
- Happy path: `AA` 报爆后遇单张 `K` 可拆出一张 `A`；若拆后只剩一张可单出的牌则自动爆，否则再次满足条件时重新确认；不能管则可过。
- Edge case: 差牌作为同点数对子回应单牌时立即结束当前回合；同样牌面作为爆牌候选时只记录对子结构，不把爆牌误判为主动差牌。
- Edge case: 连续 1 分钟无操作只把对应玩家标记为暂离；玩家恢复操作后标识消失，牌权、手牌和结算不被自动改变。
...
- Happy path: 立棍/反立、爆牌选择和拆牌后的再次确认通过协议广播；其他玩家只看到预警和公开牌面，不看到私密手牌。
- Happy path: 客户端断线后使用原会话重连，恢复原座位、剩余手牌、有效主、当前牌权和爆牌承诺；断线期间不自动过牌。
- Edge case: 连续 1 分钟无操作后公共快照出现暂离标识，恢复任何有效操作后标识消失，不生成倒计时或自动动作。
...
- 对局页以电脑端为主：本手主在左上角，中心展示公开牌，四周按上下左右展示玩家昵称、队伍、剩余牌数、名次、暂离标识和爆牌标识；AC/BD主以侧边挂件展示，当前牌权仅使用对应头像的圆形边框动效表示；玩家自己的手牌可点击选中，其他玩家手牌始终隐藏。
...
- 不显示回合倒计时；连续 1 分钟无操作时显示暂离标识，操作恢复后移除。刷新或重连时恢复到服务端快照，不重置本地选择以外的牌局状态。
...
- Happy path: 游戏页显示本手主、公开牌、双方等级、每人剩余牌数和已出完名次，不显示“当前牌权”“上一手”等文字；当前牌权仅通过头像边框表现。
- Happy path: 选择 `55` 回应单张 5 时可选择差；一对 `2` 拆一张回应单张 `A`、或一对 `A` 拆一张回应单张 `K` 时只能按正常牌权出牌，不显示差牌选项；报爆后显示本人操作状态和其他玩家头像上的“爆”标识，但不泄露剩余牌面。
- Happy path: 选中不合法牌型时出牌按钮不可提交并显示原因；服务端拒绝时保留原手牌和牌权。
- Happy path: 玩家连续 1 分钟无操作显示暂离，再次点击选牌、出牌或过牌后标识消失；页面没有倒计时和自动过牌。
...
E1–E5的用户玩法决议已同步：报爆后允许拆牌并按剩余手牌重新确认；尾家自动末位；同队前两名立即抓两家结算；未终局才执行接风；J升两级到K；立棍/反立成功仅己方升级，失败才按模式扣4/8级，最低3，J坎优先。E6–E10已由用户全部接受并纳入Task1/2/4/5/6/7，包含会话重连契约、前置规则基准、房间管理、真实浏览器E2E与资源清理。详见requirements-eng-review.md和test-plan.md。
