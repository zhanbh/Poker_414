...
export function TexasLobbyView({ snapshot, ownSeat, spectator = false, onStart, onRemove, onLeave, testMode }: {
  ...
  return (
    <main className="texas-lobby">
      ...
      <header className="texas-header"><div><h1>德州扑克房间 {snapshot.roomId}</h1><p>2—8 人无限注 · {formatTexasChips(TEXAS_STARTING_STACK)} 筹码 · 盲注 {formatTexasChips(TEXAS_SMALL_BLIND)}/{formatTexasChips(TEXAS_BIG_BLIND)}</p></div><span>等待开局</span></header>
      <RoomPurposeNotice />
